...
- `src/biocybe/cli.py` — point d'entrée `biocybe`, sous-commandes : `scan`, `quarantine list/restore`, `intel update/rules`, `tcell train/status/evaluate`, `api serve`
...
| **2.3.a — API REST + Prometheus** | ✅ | Flask + waitress/gunicorn, Bearer token, /healthz /api/v1/scan /quarantine /metrics, 20 tests |
