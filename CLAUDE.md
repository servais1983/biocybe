...
- `src/biocybe/cli.py` — point d'entrée `biocybe`, sous-commandes : `scan`, `quarantine list/restore`, `intel update/rules list/update/verify/build-cache`, `tcell train/status/evaluate`, `api serve`, `notify list/test`, `audit show/verify`, `crypto generate-key`
...
| 3.e+ — Cellule réseau | ⏳ | Watcher trafic sortant qui consomme les blocklists URLhaus/ThreatFox |
