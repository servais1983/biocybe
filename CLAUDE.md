...
- `src/biocybe/swarm/swarm_sync.py` — **immunité collective** : export/import de menaces entre nœuds via bundles signés HMAC (herd immunity), jamais de FP partagé, analyste local prioritaire. `biocybe swarm {export,import,status}`. NB : distinct du legacy `swarm_intelligence/`
