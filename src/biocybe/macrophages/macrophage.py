...
class MacrophageCell(BiologicalCell):
    ...
    def __init__(self, name: str, config: dict = None):
        ...
        self.directories_to_watch = self.config.get(
            "watch_directories",
            ["/etc", "/tmp", "/var/log"],  # noqa: S108 — chemins surveillés, pas d'écriture
        )
    ...
    def _process_cycle(self):
        ...
        if (
            ...
            or (current_time - self.last_scan_time).total_seconds() >= self.scan_interval
        ):
            self.logger.debug("Démarrage d'un scan complet")
    ...
    def _handle_system_start(self, message: CellMessage):
        ...
        self.send_message(
            ...
            target="broadcast",
            payload={"timestamp": datetime.now().isoformat(), "established_by": self.name},
        )
