    ...
    def load_database(self):
        ...
        try:
            ...
            if os.path.exists(yara_path):
                ...
                logger.info(
                    f"Chargement de {len(self.yara_rules)} informations sur les règles YARA"
                )
...
class SignatureDetector:
    ...
    def _load_config(self, config_path):
        ...
        try:
            ...
            return {"signatures": {"yara_rules_path": "rules/yara", "update_interval": 3600}}
    ...
    def _load_yara_rules(self):
        ...
        try:
            ...
            elif os.path.exists(rules_path) and rules_path.endswith((".yar", ".yara")):
            ...
            if not result.is_malicious and self.external_apis:
                for api_name, api_config in self.external_apis.items():
                    if api_name == "virustotal":
                        ext_result = self._check_virustotal(
                            file_info["sha256"], api_config["api_key"]
                        )
                        if ext_result and ext_result.get("positives", 0) > 0:
                            result.is_malicious = True
                            result.confidence = min(
                                0.99, ext_result.get("positives", 0) / ext_result.get("total", 1)
                            )
                            result.matched_rules.append(
                                f"virustotal:{ext_result.get('positives')}/{ext_result.get('total')}"
                            )
    ...
    def _get_file_info(self, file_path):
        ...
        try:
            ...
            logger.error(
                f"Erreur lors de la collecte d'informations sur le fichier {file_path}: {e}"
            )
            return {
                "error": str(e),
                "size": 0,
