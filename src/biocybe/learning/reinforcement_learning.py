...
class BioCybeRLAgent:
    ...
    def _build_policy_network(self):
        ...
        model.compile(
            optimizer=Adam(learning_rate=self.learning_rate), loss="categorical_crossentropy"
        )
    ...
    def train(self, env, episodes=None, render=False):
        ...
        for episode in range(episodes):
            ...
            if episode % 10 == 0:
                logger.info(
                    f"Episode: {episode}/{episodes}, Reward: {total_reward}, Avg Reward: {avg_reward}, Epsilon: {self.epsilon:.4f}"
                )
