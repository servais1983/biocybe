...
class SwarmNode:
    ...
    def share_information(self, peers=None):
        ...
        logger.info(
            f"Nœud {self.node_id} partage {len(shared_info)} découvertes avec {len(peers)} pairs"
        )
        return shared_info
        ...
        if output_file:
            try:
                ...
                nx.draw_networkx_nodes(
                    G, pos, node_color=["red" if n == self.node_id else "blue" for n in G.nodes]
                )
    ...
    def _merge_findings(self, existing, new):
        ...
        if "data" in new and "data" in existing:
            for key, value in new["data"].items():
                if key in merged["data"]:
                    ...
                    if new.get("timestamp", datetime.now()) > existing.get(
        ...
        if "confidence" in new and "confidence" in existing:
            ...
            merged["confidence"] = (existing["confidence"] * w1 + new["confidence"] * w2) / (
                w1 + w2
            )
        ...
        for finding in group[1:]:
            ...
            for key, value in data.items():
                if key in base_data:
                    ...
                    elif finding.get("timestamp", datetime.min) > group[0].get(
                        "timestamp", datetime.min
                    ):
    ...
    def _update_trust_score(self, source_id, shared_info):
        ...
        logger.debug(
            f"Score de confiance de {source_id} mis à jour: {current_score:.2f} -> {self.trust_scores[source_id]:.2f}"
        )
...
class AntColonyDetector:
    ...
    def visualize_paths(self, paths, output_file=None):
        ...
        try:
            ...
            for i, (path, score) in enumerate(paths[:5]):  # Limiter à 5 chemins pour la lisibilité
                ...
                path_nodes = set(path)
                nx.draw_networkx_nodes(
                    self.graph, pos, nodelist=path_nodes, node_color=color, node_size=500
                )

                # Dessiner les arêtes du chemin
                path_edges = [(path[i], path[i + 1]) for i in range(len(path) - 1)]
                nx.draw_networkx_edges(
                    self.graph, pos, edgelist=path_edges, width=3, alpha=0.7, edge_color=color
                )
    ...
    def explain_path(self, path, score):
        ...
        explanation.append("")
        explanation.append(
            "Ce chemin représente une séquence possible d'événements ou d'actions qui pourrait indiquer une propagation de menace dans le système."
        )
    ...
    def _calculate_transition_probs(self, current, neighbors):
        ...
        for i, neighbor in enumerate(neighbors):
            ...
            pheromone = self.pheromones.get(
                (current, neighbor), self.config.get("initial_pheromone", 0.1)
            )
